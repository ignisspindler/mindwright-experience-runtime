...
import { loadVocabulary, readRepoFile } from './vocabulary.js';
...
export function assembleBootstrap(source, { rung = null, label = null, returnUrl = DEFAULT_RETURN_URL, vocab = loadVocabulary() } = {}) {
  ...
  const parts = [
    ...
    part('Runtime Kernel', stripComments(readRepoFile('runtime/kernel.md')), 'KERNEL'),
  ...
  if (sealed.primary) {
    const cards = [
      cardSection(sealed.primary, 'primary'),
      ...(sealed.secondary ?? []).map(id => cardSection(id, 'secondary')),
    ];
    parts.push(part('Architecture guidance', cards.join('\n\n'), 'GUIDANCE'));
  }

  const dimensions = [];
  if (sealed.scaffolding?.length) dimensions.push(stripComments(readRepoFile('runtime/dimensions/scaffolding.md')));
  if (sealed.mechanisms?.length || sealed.operators?.length) {
    ...
    dimensions.push(`${stripComments(readRepoFile('runtime/dimensions/pressures.md'))}\n\n${entries.map(e => `- \`${e.id}\`: ${e.gloss}`).join('\n')}`);
  }
  if (sealed.expectation) dimensions.push(stripComments(readRepoFile('runtime/dimensions/expectation.md')));
  if (sealed.realization) dimensions.push(stripComments(readRepoFile('runtime/dimensions/realization.md')));
...
// bootstrap a participant reads before the divider.
